...
- [Development guidelines](#development-guidelines)
    - [General NovaNickel](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
NovaNickel uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
NovaNickel is a multi-threaded application, and deadlocks or other
...
NovaNickel can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as NovaNickel, where everyone uses
...
pay attention to for reviewers of NovaNickel code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: NovaNickel uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of NovaNickel, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against NovaNickel so that they can be integrated
...
file descriptors, it will cause problems with NovaNickel's `select()` loop, because
...
compatibility between nodes. This happened in NovaNickel 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to NovaNickel's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of NovaNickel, the wallet was always
